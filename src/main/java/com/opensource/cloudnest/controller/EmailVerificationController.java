...
public class EmailVerificationController {
    ...
    public ResDTO<Object> resendVerificationEmail(@RequestParam String email) {
        Optional<Profile> optionalProfile = profileRepository.findByEmail(email);
        if (optionalProfile.isPresent()) {
            String token = tokenService.createToken(email);
            emailService.sendSignUpLink(email, token);
            return new ResDTO<>(Boolean.TRUE, ResDTOMessage.SUCCESS, "Verification email resent successfully");
        }
        return new ResDTO<>(Boolean.FALSE, ResDTOMessage.RECORD_NOT_FOUND, "Profile not found for the provided email");
    }
